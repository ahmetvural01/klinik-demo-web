  ...
  return (
    <div ref={rootRef} className={`relative ${className}`}>
      <button
        ...
        aria-expanded={open}
        className="flex h-11 w-full items-center gap-2 border-0 border-r border-slate-200 bg-slate-50 px-3 text-left text-sm font-semibold text-slate-700 outline-none transition hover:bg-slate-100 focus-visible:bg-white focus-visible:ring-2 focus-visible:ring-primary/20"
      >
      ...
      {open && typeof document !== "undefined" && createPortal(
        <div data-phone-country-picker className="fixed z-[130] overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl" style={{ left: position.left, top: position.top, width: position.width }}>
          <div className="border-b border-slate-100 p-2">
            <label className="flex h-9 items-center gap-2 rounded-md bg-slate-50 px-2.5 text-slate-500 focus-within:bg-white focus-within:ring-2 focus-within:ring-primary/15">
          ...
          <div role="listbox" className="max-h-56 overflow-y-auto p-1.5">
            {options.map((item) => (
              <button
                ...
                className={`flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-sm transition ${item.code === value ? "bg-primary/10 text-primary" : "text-slate-700 hover:bg-slate-50"}`}
              >
