        ...
        <aside className="relative bg-[linear-gradient(160deg,#0f172a_0%,#0f2f4a_55%,#114a5f_100%)] p-8 text-white md:p-10">
          ...
          <div className="relative z-10 flex h-full flex-col justify-between">
            ...
            <div className="mt-8 grid grid-cols-1 gap-3 text-sm">
              <div className="rounded-xl border border-white/20 bg-white/10 p-3">JWT + HttpOnly cookie ile guvenli oturum yonetimi</div>
              <div className="rounded-xl border border-white/20 bg-white/10 p-3">Role-based yetkilendirme ile moduler erisim</div>
              <div className="rounded-xl border border-white/20 bg-white/10 p-3">Klinik ekipleri icin hizli ve sade giris deneyimi</div>
            </div>

            <p className="mt-8 text-xs text-slate-300">KlinikModern • Klinik Giris Paneli</p>
          </div>
        </aside>

        <form onSubmit={onSubmit} className="p-7 md:p-10">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <h2 className="text-2xl font-black text-slate-900">Klinik Girisi</h2>
            <p className="mt-1 text-sm text-slate-500">Kurum bilgisi ve personel kimligi ile devam edin.</p>
          </div>

          <div className="mt-5 space-y-4">
            <label className="block text-sm font-semibold text-slate-700">
              Kurum Kodu veya Kisa Adi
              <input
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-xs text-slate-500">Guvenli baglanti aktif</span>
            </div>
            ...
            <button disabled={loading} className="w-full rounded-xl bg-cyan-600 p-3 text-sm font-bold text-white transition hover:bg-cyan-700 disabled:opacity-50">
              {loading ? "Giris yapiliyor..." : "Panele Giris Yap"}
            </button>
          </div>
        </form>
      </div>
