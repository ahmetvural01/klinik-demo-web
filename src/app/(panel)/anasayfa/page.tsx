  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3 px-1 pt-1">
        <div>
          <h1 className="font-display text-xl font-extrabold text-slate-950">Günlük görünüm</h1>
          <p className="mt-1 text-sm text-slate-500">{dateLabel}</p>
        </div>
        <p className="pb-0.5 text-xs font-medium text-slate-400">{roleLabel} · Güncellendi {lastSyncLabel}</p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {summaryItems.map((item, idx) => (
          <Link
            key={item.id}
            href={item.href}
            className={`relative min-w-[150px] overflow-hidden rounded-xl border px-5 py-4 transition hover:-translate-y-0.5 hover:shadow-sm ${idx === 0 ? "border-primary/15 bg-primary/[0.045]" : "border-amber-200/70 bg-amber-50/65"}`}
          >
            <span className={`absolute inset-x-0 top-0 h-0.5 ${idx === 0 ? "bg-primary" : "bg-amber-500"}`} />
            <p className="text-[11px] font-semibold text-slate-500">{item.label}</p>
            <p className={`mt-1 text-2xl font-extrabold tabular-nums ${summaryValueClass[item.tone]}`}>{item.value}</p>
          </Link>
        ))}
      </div>
