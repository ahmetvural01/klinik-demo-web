...
type StockMoveForm = { type: string; quantity: string; note: string; supplier?: string; unitPrice?: string };
...
export default function StokPage() {
  ...
  async function submitNew() {
    ...
    setSaving(true);
    const res = await fetch("/api/stock", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(newItem) }).catch(() => null);
    if (!res || !res.ok) {
      const body = await res?.json().catch(() => null);
      showToastSafe({ title: "Stok eklenemedi", message: body?.message || body?.error || "Lütfen bilgileri kontrol edip tekrar deneyin.", type: "error" });
      setSaving(false);
      return;
    }
    setShowNew(false);
  ...
  async function submitMove() {
    ...
    setSaving(true);
    const res = await fetch(`/api/stock/${moveItem.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(move) }).catch(() => null);
    if (!res || !res.ok) {
      const body = await res?.json().catch(() => null);
      showToastSafe({ title: "Stok hareketi kaydedilemedi", message: body?.message || body?.error || "Lütfen miktar ve işlem bilgilerini kontrol edin.", type: "error" });
      setSaving(false);
      return;
    }
    setMoveItem(null);
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-100 bg-white p-3 shadow-sm">
        <div className="flex flex-wrap items-center gap-2">
          <h1 className="text-lg font-black text-slate-900">Stok</h1>
          <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-700">{items.length} kalem</span>
          <span className={`rounded-full px-3 py-1 text-sm font-semibold ${lowStock > 0 ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}>{lowStock} kritik</span>
        </div>
        <button onClick={() => setShowNew(true)} className="flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-slate-800">
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Toplam Kalem</p>
          <p className="mt-1 text-2xl font-black text-slate-900">{items.length}</p>
        </div>
        <div className={`rounded-2xl border p-4 shadow-sm ${lowStock > 0 ? "border-red-100 bg-red-50" : "border-emerald-100 bg-emerald-50"}`}>
          <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Kritik Stok</p>
          <p className={`mt-1 text-2xl font-black ${lowStock > 0 ? "text-red-700" : "text-emerald-700"}`}>{lowStock}</p>
        </div>
        <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Toplam Değer</p>
          <p className="mt-1 text-xl font-black text-blue-700">{CURRENCY.format(totalValue)}</p>
        </div>
        <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Kategori Sayısı</p>
          <p className="mt-1 text-2xl font-black text-slate-900">{categoryCount}</p>
        </div>
      </div>

      {/* Toolbar */}
      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-slate-100 bg-white px-4 py-4 shadow-sm">
        ...
        <div className="flex flex-wrap gap-1.5">
          {CATEGORIES.map(c => (
            <button key={c} onClick={() => setCategory(c)} className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${category === c ? "bg-slate-900 text-white shadow-sm" : "border border-slate-200 text-slate-600 hover:bg-slate-50"}`}>{c}</button>
          ))}
        </div>
      </div>
      ...
      {lowStock > 0 && (
        <div className="flex items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4">
          ...
          <p className="text-sm font-semibold text-amber-800">{lowStock} ürün kritik stok seviyesinin altında.</p>
        </div>
      )}

      <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-800 shadow-sm">
        Stok girişi satın alma/firma siparişi üzerinden yapılır. Bu ekranda ürün kartını açabilir ve klinikte tüketilen malzemeler için stok çıkışı yapabilirsiniz.
      </div>

      {/* Table */}
      <div className="rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden" aria-busy={loading}>
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-50 text-left">
                {["Malzeme","Kategori","Stok","Min. Stok","Birim","Birim Fiyat","Toplam Değer","Tedarikçi","İşlem"].map(h => (
                  <th key={h} className="px-4 py-3 text-xs font-bold uppercase tracking-wide text-slate-500">{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {filtered.map(item => {
                ...
                return (
                  <tr key={item.id} className={`transition ${low ? "bg-red-50/40 hover:bg-red-50" : "hover:bg-slate-50"}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className="rounded-full bg-blue-100 px-2.5 py-1 text-xs font-bold text-blue-700">{item.category}</span>
                    </td>
                    <td className="px-4 py-3">
                      ...
                      {low && <span className="ml-2 rounded-full bg-red-100 px-2 py-1 text-xs font-black text-red-600">Kritik</span>}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      ...
                      }} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:border-primary hover:bg-primary hover:text-white">
                        Stok İşlemi
                      </button>
                    </td>
                  </tr>
          ...
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input value={newItem.supplier} onChange={e => setNewItem(i => ({ ...i, supplier: e.target.value }))} className={inp + " w-full"} placeholder="İsteğe bağlı" />
              </div>
              <p className="text-xs text-slate-500">Bu form yeni ürün kartı açar. Satın alma girişleri firma siparişinden, kullanım kaynaklı düşüşler stok çıkışı ekranından yönetilir.</p>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="rounded-xl border border-blue-100 bg-blue-50 px-3 py-2 text-xs text-blue-700">
                Klinik tüketimi, iade dışı kullanım ve düzeltme çıkışlarını buradan kaydedin. Satın alma kaynaklı girişler firma siparişi üzerinden ilerlemelidir.
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-600">İşlem Türü</label>
                <div className="flex gap-2">
                  {["CIKIS"].map(t => (
                    <button key={t} onClick={() => setMove(m => ({ ...m, type: t }))} className={`flex-1 rounded-xl py-2.5 text-sm font-bold transition ${move.type === t ? "bg-red-500 text-white" : "border border-slate-200 text-slate-600 hover:bg-slate-50"}`}>
                      Stoktan Düş
                    </button>
                  ))}
                </div>
